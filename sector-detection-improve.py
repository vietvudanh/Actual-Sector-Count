...
for line in DF_trans.index:
    if DF_trans.at[line, 'begin_time'] > duration:
        ...
        sector_count[n_step] = sector_count[n_step] + 1
    elif DF_trans.at[line, 'begin_time'] < duration:
        if int(DF_trans.at[line, 'begin_time']/step_sec) == int(DF_trans.at[line, 'leaving_time']/step_sec):
            pass
        else:
            for i in range(math.ceil(DF_trans.at[line, 'begin_time']/step_sec),
                           math.floor(DF_trans.at[line, 'leaving_time']/step_sec)+1):
